...
!(async () => {
  ...
  // 测量上行速率
  const uploadMb = $.lodash_get(arg, 'uploadMb') || 1;
  const uploadBytes = uploadMb * 1024 * 1024;
  const uploadData = new Uint8Array(uploadBytes);
  crypto.getRandomValues(uploadData); // 生成随机数据
  ...
  const uploadSpeed = uploadMb / uploadDuration;
  // 测量下行速率
  let start = Date.now()
